...
float[nSpectralSamples] singleScatteringAndAttenuation(
  ...
  // for debugging: read the color buffer and draw the color

  // get the pixel coordinates of the current fragment
//	  ivec2 pixelPos =  ivec2 ( gl_FragCoord.xy );
//	  vec3  surfaceColor = texelFetch(uColorTex, pixelPos, 0).xyz; // color
	  
	  //return spectralColor(surfaceColor); // for debugging
     //  this surface color is NOT attenuated through the water volume
...
// for debugging above
  ...
  if ( !isHit ) { // the AABB  is not hit? 	 getBackgroundColor:
	...
	//  surfaceColor = vec3(0.5, 0.0, 0.0);
	 ...
	 // spectralColor(surfaceColor, LSpectrumOut);
  ...
  // in the direction of -rayDir
	  ...
	  float zpixel  = texelFetch(uDepthTex, pixelPos, 0).r;  // z in non linear range [0,1]
...
vec3  spectrumToRGB( float[nSpectralSamples] irainbow);
...
vec3 calculate_rainbowColor (bool isPointLight,  vec3 lightPosOrDir, vec3 rayDir, 
	...
	//return spectrumToRGB( LSpectrumSun);
...
void main() {
 //for debugging 
  ...
  /*
	 ...
	 // r = spectralColor( surfaceColor);
	
	   spectralColor( surfaceColor, r);

	  //surfaceColor = vec3(0.2, 0.3, 0.1);
    
	  if ( surfaceColor[0] < surfaceColor[1] && surfaceColor[0] < surfaceColor[2] ) {
	            r = spectralColor( surfaceColor);
      }
      else  for (int j=0; j < nSpectralSamples; j++ ) {
             r[j] = 0.0;
            }
    

	 // fragColor = vec4( spectrumToRGB(r), 1.0);
  ...
  for debugging
     ...
     int i2 =3; // Yellow  spectrum
     for (int j =0; j < nSpectralSamples; j++) {
	
          //  r[j] += rgb[0] * rgbRefl2SpectWhite[j];
		  //r[j] += rgb[0] *  texelFetch(uPrimarySpectrums, i1 * nSpectralSamples + j, 0).r;  

		    r[j] = texelFetch(uPrimarySpectrums, i2 * nSpectralSamples + j, 0).r;  

	 } // for
 
 vec3 rgbColor;
 
 rgbColor = spectrumToRGB( r);
...
*/
 ...
 /* debug: OK to read the 1D texture
 ...
 for (int i = 0; i < 7; i++ ) {
       for (int j =0; j < nSpectralSamples; j++) {
          //r[j] += rgb[0] * rgbRefl2SpectWhite[j];
		  r[j] +=   texelFetch(uPrimarySpectrums, i * nSpectralSamples + j, 0).r;  
...
// from pbrt book
//Functions in GLSL use a calling convention called "value-return." This means that values passed to functions are copied into parameters when the function is called, 
//and outputs are copied out when the function returns.//
//  the value of the parameter will be copied out into the variable that the user specified when calling the function. Note that the initial value of the parameter at the start of the function being called is undefined,
...
// primarySpectrums
  ...
  if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2]) {
     ...
     if (rgb[1] <= rgb[2]) { // rgb[0] <= rgb[1] << rgb[2]
    ...
    // Compute reflectance _SampledSpectrum_ with _rgb[1]_ as minimum
       ...
       if (rgb[0] <= rgb[2]) { // rgb[1] <= rgb[0] <= rgb[2]
	        int i2 = 2; // Magenta
			int i3 = 6; // Blue
	        for (int j =0; j < nSpectralSamples; j++) {
    ...
    // Compute reflectance _SampledSpectrum_ with _rgb[2]_ as minimum
	   ...
	   if (rgb[0] <= rgb[1]) { // rgb[2] <= rgb[0] <= rgb[1]
	      int i2 = 3;// Yellow
	      int i3 = 5; // Green
	      for (int j =0; j < nSpectralSamples; j++) {
