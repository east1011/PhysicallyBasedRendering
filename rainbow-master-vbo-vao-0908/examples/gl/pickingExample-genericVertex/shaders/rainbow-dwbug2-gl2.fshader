...
// gl_FragCoord = (x,y,z,1/w) => NDC can be obtained: w: perspective division
   ...
   // for debug	
	...
	int tj;
    for (tlambda  = lambdaStart, tj = 0; tlambda  < lambdaEnd; tlambda += lambdaStep, tj++) {
        particlePhase[tj] = computePhaseFunction( uRadius,  tlambda, theta); //  lambda in lambdaStep *2
    }
	...
	if ( tlambda >= lambdaEnd ) lambdaIndex = nSpectralSamples - 1;
	else {
...
/*
 ...
 if ( !( tpsi >=  130 &&  tpsi < 142 ) ) { 
	...
	float tempPsi = thetaStart + (thetaEnd - thetaStart) * x;
    ...
    float lambda = lambdaStart + ( lambdaEnd - lambdaStart) * y;
	
	int lambdaIndex;

	if ( lambda >= lambdaEnd ) {
	  lambdaIndex = nSpectralSamples - 1;
	  
	}

	else {
	  lambdaIndex = int(  (lambda - lambdaStart) /  lambdaStep *2 ) ; 
	...
	for (lambda = lambdaStart, j =0; lambda < lambdaEnd; lambda += lambdaStep*2, j++) {
		...
		int i =   int ( (lambda - 380 ) / lambdaStep*2 );
  ...
  return;
   ...
   // for debug	
    ...
    for (lambda = lambdaStart, j = 0; lambda < lambdaEnd; lambda += lambdaStep*2, j++) {
        ...
        particlePhase [j] = computePhaseFunction( uRadius,  lambda, theta); //  lambda in lambdaStep *2
