...
const int nSpectralSampleSteps = 29;
...
float[nSpectralSamples] singleScatteringAndAttenuation( vec3 surfaceColor, float zEye, 
  ...
  // in the direction of -rayDir
	  ...
	  float tmaxAtten; // the t value at which attenuation begins
        ...
        for (float t = tmaxAtten; t >= tmin; t-= deltaT  ) { 
		      ...
		      for ( int j =0; j < nSpectralSamples; j++) {			  
                 ...
                 LSpectrumOut[j] += exp( - sigma_e * ( t - tmin) ) * sigma_s  
		                            * avgPhaseFunction(lambda, psi) * LSpectIn[j];
                 lambda += lambdaStep;
...
vec3 calculate_rainbowColor (vec3 surfaceColor, float zEye, bool isPointLight,  vec3 lightPosOrDir, vec3 rayDir, 
...
// singleScatteringAndAttenuation( vec3 surfaceColor, float zEye, 
//                              bool isPointLight, vec3 lightPosOrDir, float[nSpectralSamples] LSpectrumSun,
//	                          vec3 rayDir, float sigma_s, float sigma_e ) 
	irainbow  = singleScatteringAndAttenuation(surfaceColor, zEye, 
...
rgbRefl2SpectBlue = blueSpectrum();
 ...
 // and get the RGB color from the spectrum
   ...
   if ( isPointLight ) {
	     ...
	     volumeColor = calculate_rainbowColor ( surfaceColor, zEye, isPointLight, lightPosOrDir,  rayDir, sigma_s, sigma_e); 
