...
vec3 oldToRGB( float [nSpectralSamples] irainbow ) {
	...
	// sun_{XYZ} = ( 94.109, 97.2861, 108.776) [ Wm^{-2}sr^{-1} ] integrated value from 400nm to 700nm
	/* 
	...
	*/
	...
	vec3 rgbColor = uXYZtosRGB * XYZColor;
...
float attenFactor(vec3 currPos) {
	...
	*/
	...
	//if x = 2.3, exp(-x) = 0.1
	//if x = 0.7, exp(-x) = 0.5 
	//float hRange = deviationFromHeight0 / heightRangeFromCenter * 2.3;
	//float wRange = deviationFromCenterWidth / widthRangeFromCenter * 0.51;
	//float dRange = deviationFromCenterDepth / depthRangeFromCenter * 0.51;
	float hRange = deviationFromHeight0 / heightRangeFromCenter * 2.31;
	float wRange = deviationFromCenterWidth / widthRangeFromCenter * 0.51;
	float dRange = deviationFromCenterDepth / depthRangeFromCenter * 0.51;

	float sqrtRange = sqrt(pow(hRange, 2.0) + pow(wRange, 2.0) + pow(dRange, 2.0));

	//if (sqrtRange < 1.21) { // if sqrtRange < 1.21, exp(-sqrtRange) < 0.3
	//	sqrtRange = 1.21;
	//}

	float attenuationFactor = exp( - sqrtRange );
...
float[nSpectralSamples] calculate_radiance_of_sun_Plank() {
    ...
    float k = 1.3806e-23;      // Boltzmann's constant
   // float T = 5782;            // Sun's temperature in Kelvin
   float T = 6504; // corresponds to D65: http://en.wikipedia.org/wiki/Illuminant_D65
    float Lsun;
...
float sumOfAttenFactor(float tmax, float tmin, vec3 rayDir) {
	int N = 20;
	float deltaT = (tmax - tmin) / float(N);

	float cumulAttenfactor = 0.0;
	...
	for ( int i = N; i >= 1; i-- ) {
		...
		float attenFactorAtCurrPos = attenFactor(currPos);
		//float currPosSigma_e = extCrossSection  * dropDensityAtCurrPos;			// assume the original sigma_e is the value on the ground where
		//																			// height is zero 
		
		cumulAttenfactor += attenFactorAtCurrPos * deltaT;

		//return sumExtinction; 				   
	} // for t: all positions on the current ray	

	return cumulAttenfactor;
} // sumOfDropDensity


float sumOfDropDensity(float tmax, float tmin, vec3 rayDir) {
	int N = 20;
	float deltaT = (tmax - tmin) / float(N);

	float cumulDropDensity = 0.0;
	...
	for ( int i = N; i >= 1; i-- ) {
		...
		cumulDropDensity += dropDensityAtCurrPos * deltaT;

		//return sumExtinction; 				   
	} // for t: all positions on the current ray	

	return cumulDropDensity;
} // sumOfDropDensity
...
vec3 singleScatteringAndAttenuation(float radius, vec3 surfaceColor, float zEye, 
	...
	//spect1 = vec4( -1.0, -1.0, -1.0, -1.0 );
	//spect2 = vec4( -1.0, -1.0, -1.0, -1.0 );
	//spect3 = vec4( -1.0, -1.0, -1.0, -1.0 );
	//spect4 = vec4( -1.0, -1.0, -1.0, -1.0 );
	spect1 = vec4( 0.0, 0.0, 0.0, 0.0 );
	spect2 = vec4( 0.0, 0.0, 0.0, 0.0 );
	spect3 = vec4( 0.0, 0.0, 0.0, 0.0 );
	spect4 = vec4( 0.0, 0.0, 0.0, 0.0 );

	float[nSpectralSamples] LSpectrumOut, LSpectrumOut0;
	
	
	vec3 rayDirInBoxFrame = vec3( uInvModelMatrixAABB * uEyeMatrix * vec4(rayDir, 0.0) );
	...
	if ( !isHit ) { // the AABB is not hit? 	 getBackgroundColor:
		...
		if ( isPointLight ) { // point light
			...
			//else {	// psi is a rainbow scattering angle
				...
				float cumulDropDensityFromSurface = sumOfDropDensity( tmaxAtten, tmin, rayDir );
				float cumulAttenFactorFromSurface = sumOfAttenFactor( tmaxAtten, tmin, rayDir );

                // for debugging
				//spect2 = vec4( tmin, tmax, opticalDepthFromSurface, exp(- opticalDepthFromSurface) );
				spect2 = vec4( cumulDropDensityFromSurface, cumulAttenFactorFromSurface, opticalDepthFromSurface, exp(- opticalDepthFromSurface) );
