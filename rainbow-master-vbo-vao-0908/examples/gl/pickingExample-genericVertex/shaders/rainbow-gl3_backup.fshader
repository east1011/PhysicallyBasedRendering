...
bool rayAABBIntersect ( Ray ray,  vec3[2] aabb,
                        out float tmin, out float tmax,  vec3 normalAtFront );
...
bool rayOBBIntersect ( vec3 uEyeOriginInBoxFrame, vec3 rayDirInBoxFrame, vec3 minPosInBoxFrame, 
                        vec3 maxPosInBoxFrame, out float tmin, out float tmax, vec3 normalAtFront) {
 ...
 return rayAABBIntersect (ray, AABB,  tmin,  tmax,  normalAtFront );
...
void main() {
  ...
  // the normal vector to the water volume in eye space

  normalAtFront = vec3(0,0,1);
  
  // uAABBmin and uAABBmax are  in world space

   vec3[2]  uAABB = vec3[2](uAABBmin, uAABBmax);  // shoud be: uAABBmin < uAABBmax
  ...
  bool isHit =  rayOBBIntersect ( uEyeOriginInBoxFrame, rayDirInBoxFrame, 
                                  uAABBmin, uAABBmax, tmin, tmax,  normalAtFront);
  
  if ( !isHit ) { // the AABB  is not hit? But the ray always hit, because the fragment shader  
                        // will be called only for the fragment seen by the camera rays. So, if every thing does well, 
						// this block will not be executed. Just added for debugging.
  ...
  else { //  hit the front and the back face of AABB box; the ray always hits because
	         // the shader is called only for the fragments that are seen by the camera, and the ray is shot
			 // in the direction of those fragments
			 // uSunRayDir points to the sun
	 
	
	 //vec3 lightDir = uSunRayDir; // the direction to the light for day rainbow
	 vec3 lightDir = normalize( uLight1Pos - vPosition ) ; // the direction to the light for night bow
	  	  
     float psi = acos ( dot ( -rayDir, -lightDir ) ) * 180.0 / pi; // -lightDir = light propogation direction
	                                                               // -rayDir = the direction of the light scattered to the 
																     // pixel
    ...
    // get the angle between the observer ray and the front face of the AABB
         ...
         phi_sun = acos( dot( normalAtFront,  lightDir )  );

	// compute the optical depth normal to the AABB
	// tmin is the distance to the front surface of  AABB
    ...
    // volume space between tmin and tmax.
	   ...
	   }  // within the psi data available  

	   else { // no scattering angles psi for rainbow formation.

	    fragColor  = vec4( 0,0,1, 1); // for bebugging
		//discard;  the background color is maintained.
