...
void main() {
  ...
  // Compute the coordinates of AABB with respect to the view space

  vec3 vAABBmin = vec3( uInvEyeMatrix * vec4( uAABBmin, 1) );
  vec3 vAABBmax = vec3( uInvEyeMatrix  * vec4( uAABBmax, 1) );

  vec3[2]  uAABB = vec3[2](vAABBmin, vAABBmax);  // shoud be: vAABBmin < vAABBmax
  ...
  vec3 rayDir = normalize( vPosition ); // vPosition is on the front of AABB box
  vec3 viewOrigin  = vec3(0,0,0);
     
  vec3 sunRayDirInView = vec3( uInvEyeMatrix * vec4(uSunRayDir, 0.0) );
  	  
  float psi = acos ( dot ( -rayDir, -sunRayDirInView ) ) * 180.0 / pi;

  if (  psi >=  136 &&  psi <= 141    ) { 
  
    
      Ray ray = makeRay( viewOrigin, rayDir);
	  
	   
	 	 
	 int hitType = intersection_distance_no_if( ray, uAABB, tmin, tmax, normalAtFront );	

      if ( hitType == 0 ) { // the AABB  is not hit => do not paint the current fragment: This case cannot happen
	  ...
	  else { // hitType ==1: hit the front and the back face of AABB box
