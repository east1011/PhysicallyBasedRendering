...
//You can use **loop indices or constants** [== contant-index-expression] to index into arrays. 
 ...
 // return r;
  ...
  if (rgb[0] <= rgb[1] && rgb[0] <= rgb[2]  && rgb[1] <= rgb[2] ) {
                          // rgb[0] <= rgb[1] << rgb[2], case (1.1)	  // cyan // rainbow FBO all colors in this case?
...
// surfaceColor = vec3(0.0, 1.0, 1.0); // it works

 float [nSpectralSamples] spectrum;

 spectrum = spectralColor( surfaceColor);

    //fragColor = vec4( toRGB( rgbRefl2SpectMagenta ), 1.0 );

     //fragColor = vec4( oldToRGB( rgbRefl2SpectMagenta ), 1.0 );
    // return;
	
     

  fragColor = vec4( toRGB( spectrum ), 1.0 );

 // fragColor = vec4( oldToRGB( spectrum), 1.0 );
  return;
