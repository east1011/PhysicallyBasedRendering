...
{  
	...
	*/
	...
	//gl_FragColor =  vec4( texture2D( uScatTex, vec2(indexPos)  ).r * 1.0e+6,  0,0,1);
	//return;
	...
	for( int i = 0; i < MAX_ITERATIONS; ++i ) {
		...
		if( d < uTolerance ) {	// ray hit the surface
			...
			gl_FragColor = vec4( rayLength/ maxRayLength, 0,0,1); // paint red with different intensity
	...
	gl_FragColor = vec4 (0,1,0,1);	// paint green  for debug
	return;
