...
struct UberMaterial {  
	   ...
	   int KrTextureClass; // TEX_CONST =1, TEX_IMAGEMAP =2
       int KrTextureUnit;  // index to the constTextures or the textures
	   int KtTextureClass; // TEX_CONST =1, TEX_IMAGEMAP =2
       int KtTextureUnit;  // index to the constTextures or the textures
	   int opacityTextureClass; // TEX_CONST =1, TEX_IMAGEMAP =2
       int opacityTextureUnit;  // index to the constTextures or the textures
	   int roughnessTextureClass; // TEX_CONST =1, TEX_IMAGEMAP =2
       int roughnessTextureUnit;  // index to the constTextures or the textures
	   int etaTextureClass; // TEX_CONST =1, TEX_IMAGEMAP =2
       int etaTextureUnit;  // index to the constTextures or the textures
...
uniform UberMaterial uUberMaterial;
...
vec3   evaluateOpacity( UberMaterial uUberMaterial, int opacityTextureClass, vec2 vTexCoord) {
  if ( opacityTextureClass == TEX_CONST ) {
       return uConstTextures[ uUberMaterial.opacityTextureUnit ];
  }
  else {
      return vec3( texture( uTextures[ uUberMaterial.opacityTextureUnit ], vTexCoord) );
  }
}
	
vec3 evaluteKd( UberMaterial uUberMaterial, int KdTextureClass, vec2 vTexCoord) {

 if ( KdTextureClass == TEX_CONST ) {
       return uConstTextures[ uUberMaterial.KdTextureUnit ];
  }
  else {
      return vec3( texture( uTextures[ uUberMaterial.KdTextureUnit ], vTexCoord) );
  }
}


float  evaluateEta( UberMaterial uUberMaterial, int etaTextureClass, vec2 vTexCoord) {

  if ( etaTextureClass == TEX_CONST ) {
       return uConstTextures[ uUberMaterial.etaTextureUnit ];
  }
  else {
      return vec3( texture( uTextures[ uUberMaterial.etaTextureUnit ], vTexCoord) );
  }

}
	
vec3 evaluateKs( UberMaterial uUberMaterial, int KsTextureClass, vec2 vTexCoord) {
  if ( KsTextureClass == TEX_CONST ) {
       return uConstTextures[ uUberMaterial.KsTextureUnit ];
  }
  else {
      return vec3( texture( uTextures[ uUberMaterial.KsTextureUnit ], vTexCoord) );
  }
}
	

float  evaluateRoughness (UberMaterial uUberMaterial, int roughnessTextureClass, vec2 vTexCoord ) {

  if ( roughnessTextureClass == TEX_CONST ) {
       return uConstTextures[ uUberMaterial.roughnessTextureUnit ][0]; // return the first component of the 
	                                                                   // constant texture value
  }
  else {
      return texture( uTextures[ uUberMaterial.roughnessTextureUnit ], vTexCoord);
  }
}
...
vec3  evaluteKr (UberMaterial uUberMaterial, int KsTextureClass, vec2 vTexCoord) {
  if ( KsTextureClass == TEX_CONST ) {
       return uConstTextures[ uUberMaterial.KsTextureUnit ];
  }
  else {
...
vec3 evaluateKt( UberMaterial uUberMaterial, int KsTextureClass, vec2  vTexCoord) {
  if ( KsTextureClass == TEX_CONST ) {
       return uConstTextures[ uUberMaterial.KsTextureUnit ];
  }
  else {
...
vec3 SpecularTransmission_Sample_f( SpecularTransmission specularTransmission , vec3 wo, out vec3 wi ) {
 ...
 // Figure out which $\eta$ is incident and which is transmitted
    ...
    return   vec3( (ei*ei)/(et*et) * (vec3(1.0) - F ) * specularTransmission.T / AbsCosTheta(wi)  );

} // SpecularTransmission_Sample_f()
...
vec3  bsdf_f( BSDF bsdf, vec3 vPos, vec3 vNormal, vec2 vTexCoord, vec3 woW, vec3 wiW) {
	...
	bool ignoreReflection =false;
	bool ignoreTransmission = false;

	if ( dot( wiW, bsdf.ng) * dot ( woW, bsdf.ng) > 0 )
	     ignoreReflection = true;
    else ignoreTransmission = true;

    vec3 f  = vec3(0.0, 0.0, 0.0);

	vec3  op = evaluateOpacity( uUberMaterial, uUberMaterial.opacityTextureClass, vTexCoord);

	if ( ! ignoreTransmission ) {
      ...
      if ( op != vec3(1.) ) {
        ...
        SpecularTransmission specularTransmission  = SpecularTransmission( -op + vec3(1.), ei, et,  fresnel  );
																		    // 1 - op = transmission scale factor
        f += SpecularTransmission_Sample_f( specularTransmission, wo, wi );
      }
	 }


    vec3 kd = op * evaluteKd( uUberMaterial, uUberMaterial.KdTextureClass, vTexCoord);
    ...
    float eta = evaluateEta( uUberMaterial, uUberMaterial.etaTextureClass, vTexCoord);
	
    vec3 ks = op * evaluateKs( uUberMaterial, uUberMaterial.KsTextureClass, vTexCoord);
	
    if (ks != vec3(0,0,0) ) {
        Fresnel fresnel =  Fresnel(eta, 1.0);
        float rough = evaluateRoughness (uUberMaterial, uUberMaterial.roughnessTextureClass, vTexCoord );
        Microfacet microFacet  =  Microfacet( ks, fresnel,  Blinn( 1.f / rough ) );
        f +=  Microfacet_f ( microFacet, wo, wi );
    }

	if ( !ignoreReflection ) { 
      vec3  kr = op * evaluteKr (uUberMaterial, uUberMaterial.KsTextureClass, vTexCoord);
      if (kr != vec3(0,0,0) ) {
        Fresnel fresnel = Fresnel(eta, 1.0);
		SpecularReflection specularReflection = SpecularReflection(kr, fresnel);
		
        f += SpecularReflection_Sample_f( specularReflection, wo, wi  );
      }
    }

	if (!ignoreTransmission ) { 
      vec3 kt = op * evaluateKt( uUberMaterial, uUberMaterial.KsTextureClass, vTexCoord);
      if (kt != vec3(0,0,0) ) {
	   Fresnel fresnel = Fresnel( eta, 1.f);
	   SpecularTransmission specularTransmission = SpecularTransmission(kt, eta, 1.f, fresnel);

       f += SpecularTransmission_Sample_f( specularTransmission , wo, wi ); // kt = transmission scale factor
...
//vTBNMat = mat3(aTangent, aBinormal, aNormal);
...
//vec4 kd = texture(uTexture, vTexCoord);
