...
// Declare an empty uniforms
  ...
  for ( int i  = 0; i <g_objectList.size() - 1 ; i++ ) { // (5) This loop goes over the list of objects g_objectList and draw each object in the list.
    ...
    Cvec4 pickColor = g_objectList[i]->pickColor; // sRGB color is written to the framebuffer; linear RGB when read from the buffer
...
void testApp::setupFBO() {
    ...
    // GLuint fbo;
	 ...
	 try {
	  ...
	  checkGlErrors("in glBindFramebuffer");
	 }
...
// Declare an empty uniforms
 ...
 // extraUniforms.put("uSunRayDir", g_sunRayDir );
  ...
  extraUniforms.put("uWindowHeight",g_windowHeight);
   ...
   //readPixels();
   // draw rainbow object "rainbow": use the color texture and the depth texture attached to FBO,
   // send them to the shader for writing to the system provided buffer.
	...
	extraUniforms.put("uColorTex", 
		  shared_ptr<ColorTexture_FROM_FBO>(new ColorTexture_FROM_FBO( g_colorTex, g_windowWidth, g_windowHeight) ));
	extraUniforms.put("uDepthTex", 
		   shared_ptr<DepthTexture_FROM_FBO>(new DepthTexture_FROM_FBO(g_depthTex, g_windowWidth, g_windowHeight)));
	...
	// draw rainbow
	 try {
      ...
      Matrix4 MVM = g_invEyeRbt * (  *( g_objectList[ rainbowIndex] -> objectRbt ) ) ; // g_currentPickedObject->objectRbt may have been
	                                                              // changed by picking
...
} // drawObjects() for ordinary rendering
...
//(I'm not saying this is not an important issue)
 ...
 // extraUniforms.put("uSunRayDir", g_sunRayDir );
  ...
  extraUniforms.put("uLightRGBColor", g_lightRGBColor);
	...
	// only the default framebuffer will be visible on your screen, 
...
//		                       Matrix4::makeTranslation( 	Cvec3( 0, g_AABBsize[1]/2 + 6.4, -g_AABBsize[2]/2.0 ) ) ) );
  ...
  //shared_ptr<Geometry> groundGeometry  ( new Geometry( &vtxGround[0], &idxGround[0], 4, 6,  &sqTex[0], numOfTexCoords  )  );
  shared_ptr<Geometry> quadGeometry  ( new SimpleIndexedGeometryPNTBX( &vtxQuad[0], &idxQuad[0], vbLen, ibLen, GL_TRIANGLES  )  );
  ...
  g_objectList.push_back( rainbow );
