...
// set the parameters for the camera, which define the internal working of the camera.
	...
	float distanceToEye =  g_AABBsize[2] *1.5;  
...
//	  .put("aNormal", 3, GL_FLOAT, GL_FALSE, offsetof(VertexPN, n));
 ...
 // int ibLen, vbLen;
  ...
  makeCube(30,6.4,5, vtxCube1.begin(), idxCube1.begin() ); // fill vtxCube and idxCube, starting
...
//  shared_ptr<SgRbtNode> groundRbt ( &SgRbtNode::makeTranslation( Cvec3(0,0,0) ) ); 
 // shared_ptr<SgRbtNode>  cubeRbt ( &SgRbtNode::makeTranslation( Cvec3(5.0,0,0)  ) );
  ...
  shared_ptr<SgRbtNode> cubeRbt1 ( new SgRbtNode( SgRbtNode::makeTranslation( Cvec3(0, 3.2, 12.5)  ) )  );
 // shared_ptr< SgRbtNode> sphereRbt ( new SgRbtNode ( SgRbtNode::makeTranslation( Cvec3(-0.5,0,0) ) ) );
   shared_ptr<SgRbtNode> cubeRbt2 ( new SgRbtNode( SgRbtNode::makeTranslation( Cvec3(30, 6.35, 12.5)  ) )  );
  
  shared_ptr<Object> ground ( new Object( "ground", groundRbt, groundGeometry, g_bumpFloorMat) );
  shared_ptr<Object> cube1 ( new Object( "cube1", cubeRbt1, cubeGeometry1, g_redDiffuseMat) );
...
//messageFile << "invModelMatrixAABB:" << invModelMatrixAABB << endl;
 ...
 shared_ptr<Geometry> AABBGeometry (  new SimpleIndexedGeometryPNTBX( &vtxAABB[0], &idxAABB[0], vbLen, ibLen, GL_QUADS ) ); 
  ...
  // g_rainbowMat->getUniforms().put("uDistTex", shared_ptr<ShaderImageTexture3D_RF_RF>(new ShaderImageTexture3D_RF_RF("rainbow/distancefield_1.txt")));
   

  
	//shared_ptr<SgRbtNode> g_AABBRbt  ( new SgRbtNode ( g_cameraRotMat * 
	//	                                                Matrix4::makeTranslation( 	g_AABBcenterInViewSpace  ) ) ); 
	//g_AABBRbt.reset(  new SgRbtNode ( g_cameraRotMat * Matrix4::makeTranslation( 	g_AABBcenterInViewSpace  ) ) ); 
	
	// the center of AABB box is set to g_AABBcenter in  the world coordinate and its axes aligne with the initial camera  orientation

	g_AABBcenter = Cvec3( 0, g_AABBsize[1] / 2.0 + 6.4, 12.5);

	g_AABBRbt.reset(  new SgRbtNode (  Matrix4::makeTranslation( 	g_AABBcenter  ) ) );
	 
	// the global coordinates of the min and max corners of AABB box: AABB box is on the ground y =0
  ...
  /* AABBrainbow is not pickable object, used only to draw rainbow
  id= 0 ; // set the rainbow AABB box id to zero. // AABB box can be picked and moved.
  id = id << 8;  
  id = id | 255; // 
  pickColor  = g_picker.idToColor(id );

  AABBrainbow->pickColor = pickColor;
  ...
  */
  ...
  //g_picker.addToMap( id, AABBrainbow );
 }
...
*/
  ...
  case OF_KEY_ALT:  // debug mode, render to texture and print the content
	  renderToFbo();
	  break;
