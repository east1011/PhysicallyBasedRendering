...
void testApp::drawStuff(bool picking) {
	...
	if (!picking) { // not picking mode but an ordinary drawing mode
		...
		// render to FBO except rainbow
		int rainbowIdx = g_objectList.size() - 1;
		renderToFbo();
		renderRainbowToFrameBuffer(rainbowIdx);
...
void testApp::renderRainbowToFrameBuffer(int idx)
{
		...
		*/
		...
		// draw
		try {
			g_objectList[idx]->draw(extraUniforms);
			...
			checkGlErrors(" g_objectList[idx]->draw( ) in drawStuff()");
...
void testApp::renderToFbo() {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, d_windowWidth, d_windowHeight, 0, GL_DEPTH_COMPONENT, GL_FLOAT, 0);
	...
	//////////////////////////////////
	//// read from frame buffer			//	this should be go into frag shader
	//////////////////////////////////
	//
	//GLubyte *rgbPixels;
	//float *depthPixels;
	//
	//rgbPixels = (GLubyte*)malloc(d_windowWidth * d_windowHeight * sizeof (GLubyte)* 3); //RGB..  don't really care for the A
	//depthPixels = (float*)malloc(d_windowWidth * d_windowHeight * sizeof (float));

	//memset(depthPixels, 0, d_windowWidth * d_windowHeight * sizeof (float));
	//memset(rgbPixels, 0, d_windowWidth * d_windowHeight * sizeof (GLubyte)* 3);

	//glBindFramebuffer(GL_FRAMEBUFFER, debugTex);
	//glReadBuffer(GL_COLOR_ATTACHMENT0);
	//glReadPixels(0, 0, d_windowWidth, d_windowHeight, GL_RGB, GL_UNSIGNED_BYTE, (void*)rgbPixels);

	//glBindRenderbuffer(GL_RENDERBUFFER, rboId);
	//glReadBuffer(GL_DEPTH_ATTACHMENT);
	//glReadPixels(0,, 0, d_windowWidth, d_windowHeight, GL_DEPTH_COMPONENT, GL_FLOAT, (void*)depthPixels);
	...
	for (int i = 0; i < numOfObjects; i++) { // (5) This loop goes over the list of objects g_objectList and draw each object in the list.
		//     Where and how is this object list created?
