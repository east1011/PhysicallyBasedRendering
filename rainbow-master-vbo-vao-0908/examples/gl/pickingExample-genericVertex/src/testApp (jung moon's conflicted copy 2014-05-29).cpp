...
// x, y: the current mouse position after this little  motion is finished.
  ...
  */
  ...
  if ( !g_picked_mode ) { // move the camera
   ...
   else if (  ofGetKeyPressed( OF_KEY_SHIFT )   )  { //  rotate the camera
   ...
   cout << "move the picked object" << endl;

  //  g_picked_mode An object has been picked by pressing the  mouse

   if (   ofGetKeyPressed( OF_KEY_CONTROL  ) )   { // translate the selected object
	 switch ( button ) {
	  case OF_MOUSE_BUTTON_LEFT:
		   //cout  << " left button moves" << endl;
		   m = Matrix4::makeTranslation( Cvec3( dx, dy, 0.0) * 0.01 ); 
    ...
    else if (  ofGetKeyPressed( OF_KEY_SHIFT ) )  {  //  rotate the selected object
...
void testApp::mouseReleased(int x, int y, int button){
	...
	if ( g_picked_mode ) { // release the picked mode
		g_picked_mode = false;
		cout << "picking released" << endl;
