...
#include "GL/glew.h" // which includes <GL/gl.h> already
...
#define checkGlErrors() _check_gl_error(__FILE__, __LINE__)
...
public:
  GlShader(GLenum shaderType) {
    ...
    if (handle_ == 0) {
      ...
      std::cout << "glCreateShader fails" << std::endl;
	  messageFile << "glCreateShader fails" << std::endl;
	}
    checkGlErrors();
  }
...
public:
  GlProgram() {
    ...
    if (handle_ == 0) {
      //throw std::runtime_error("glCreateProgram fails");
		std::cout << "glCreateProgram fails" << std::endl;
		messageFile <<  "glCreateProgram fails" << std::endl;
  ...
  operator GLuint() const { // type conversion operator: (GLunit) program or GLunit( program ) returns handle_ where GlProgram program
    return handle_;
  }
};
...
public:
  ...
  // cast operator that casts GlTexture object to  GLuint so that it can be used in  glBindTexture(GLunit).
  operator GLuint () const {
    return handle_;
  }
};
...
inline void safe_glUniform1fv(const GLint handle, const GLsizei count, const GLfloat a[]) {
  if (handle >= 0)
    glUniform1fv(handle, count, a);
}

inline void safe_glUniform2fv(const GLint handle, const GLsizei count,  const GLfloat b[]) {
  if (handle >= 0)
    glUniform2fv(handle, count, b);
}

inline void safe_glUniform3fv(const GLint handle, const GLsizei count, const GLfloat c[]) {
  if (handle >= 0)
    glUniform3fv(handle, count, c);
}

inline void safe_glUniform4fv(const GLint handle, const GLsizei count,  const GLfloat d[]) {
  if (handle >= 0)
    glUniform4fv(handle, count, d);
}
