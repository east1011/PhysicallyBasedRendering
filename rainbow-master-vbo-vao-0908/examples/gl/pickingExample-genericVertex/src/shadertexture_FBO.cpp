...
ShaderImageTexture2D_RGB_RGB::ShaderImageTexture2D_RGB_RGB(const char* ppmFileName, bool srgb): tex() {
  ...
  //If it is mip-mapped and running in a fragment shader, the LOD computed by the implementation
...
//texture is used.
  ...
  if( strstr(ppmFileName,".ppm") !=NULL){
	...
	if ( !g_Gl2Compatible ) glGenerateMipmap(GL_TEXTURE_2D);
   ...
   // glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    ...
    checkGlErrors("ImageTexture::ImageTexture");
		...
		if ( colorMode == 4 ) {
		  ...
		  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA , tga.imageWidth, tga.imageHeight,
		  ...
		  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB , tga.imageWidth, tga.imageHeight,
...
ColorTexture_FROM_FBO::ColorTexture_FROM_FBO(GLuint texId): tex(texId)  { 
// needs to be reimplemented
...
DepthTexture_FROM_FBO::DepthTexture_FROM_FBO(GLuint texId): tex(texId) {
// The following is not member variables of the class, whereas tex is.
