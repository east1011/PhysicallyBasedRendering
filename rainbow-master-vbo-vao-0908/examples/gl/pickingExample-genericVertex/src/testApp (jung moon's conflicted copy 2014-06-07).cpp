...
double  g_frustMinFov = 120.0;  // A minimal of 100 degree field of view for rainbow viewing
...
static const Cvec3 g_AABBmaxLocal( rainbowVolumeWidth / 2.0,  rainbowVolumeHeight , 0.0);
static  const Cvec3 g_AABBminLocal( -rainbowVolumeWidth / 2.0, 0.0, -rainbowVolumeDepth);
...
static const   Cvec3 g_AABBsize = g_AABBmaxLocal - g_AABBminLocal;
...
// set the parameters for the camera, which define the internal working of the camera.
	...
	float volumeDepth = g_AABBsize[2]; 
	...
	g_AABBFrameInViewSpace = Cvec3( 0, g_AABBsize[1]/2.0, -g_AABBsize[2]/ 2.0 );
...
//messageFile << "invModelMatrixAABB:" << invModelMatrixAABB << endl;
 ...
 shared_ptr<Geometry> AABBGeometry (  new SimpleIndexedGeometryPNTBX( &vtxAABB[0], &idxAABB[0], vbLen, ibLen ) ); 
  ...
  id= 0 ; // set the rainbow AABB box id to zero. 
  id = id << 8;  
  id = id | 255; // 
  pickColor  = g_picker.idToColor(id );
