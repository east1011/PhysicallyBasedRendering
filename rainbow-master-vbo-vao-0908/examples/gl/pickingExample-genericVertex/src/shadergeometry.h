...
class Geometry {
public:
	Geometry( const string geoName): geoName (geoName) {}
  ...
  virtual void draw(int attribIndices[]) = 0; // to be implemented by children classes, BufferObjectGeometry

  virtual ~Geometry() {}

  const string geoName;
};
...
class Object {
public:
	shared_ptr<SgRbtNode> objectRbt;
	...
	Object( string objName, shared_ptr<SgRbtNode> objectRbt, shared_ptr<Geometry> geometry, shared_ptr<MaterialShader> material ) : 
	  objectName (objName), objectRbt (objectRbt), geometry (geometry), material  (material) 
	{ }
	  // here objectRbt and geom are references to existing data, and these data should be global, as long 
	  // as the created object is in use. 
...
public:
  ...
  VertexFormat& put(const std::string& name, GLint size, GLenum type, GLboolean normalized, int offset) {
    ...
    return *this; // The result of put() is a reference variable to *this, which is maintained after the put method is finished.
...
public:
  ...
  FormattedVbo(const VertexFormat& vertexFormatDesc)  // formatDesc = Vertex::FORMAT is of VertexFormat class
	                                                  // where Vertex is VertexPN, VertexPNX, VertexPNBTX

    : format_(  vertexFormatDesc ), length_( 0 ) {}

  // formatDesc= VertexPN::FORMAT = VertexFormat( sizeof(VertexPN) ).put("aPosition", 3, GL_FLOAT, GL_FALSE, offsetof(VertexPN, p))
  ...
  // to upload different data multiple times
   ...
   void upload(const Vertex* vertices, int length, bool dynamicUsage = false) { 
	  // Vertex = VertexPN, VertexPNX, VertexPNBTX

	assert( sizeof(Vertex)  == format_.getVertexSize() );

    glBindBuffer(GL_ARRAY_BUFFER, *this); // object *this is type-converted to buffer id.

    length_ = length;

    const int size = sizeof(Vertex) * length;  // the total size of the data in byte
	                                           // Here, Vertex is VertexPN, VertexPNX, or VertexPNBTX
...
// to call glBindBuffer only once for each distinct  FormattedVbo it wires to.
...
public:
  // Initialize, with zero vertex buffer and index buffer attached. Defaults to TRIANGLES primitive
	BufferObjectGeometry(const string geoName);
  ...
  BufferObjectGeometry& wireAttribute (const std::string& targetAttribName,
  ...
  BufferObjectGeometry& wireAttribute (std::tr1::shared_ptr<FormattedVbo> source, const std::string& sourceAttribName);

  // Declares and maps all vertex attributes contained in the 'source' FormattedVbo.
  // Same names are used for each attribute.
  BufferObjectGeometry& wireAttributes (std::tr1::shared_ptr<FormattedVbo> source);
  ...
  virtual const std::vector<std::string>& getVertexAttribNames(); 
       // getVertexAttribNames() is a reference  to the returned object, which is vertexAttribNames_, a member variable of
       // BufferObjectGeometry class

  virtual void draw(int attribIndices[]); // inherited from parent Geometry
...
private:
  ...
  bool wiringChanged_;
  Wiring wiring_; // BufferObjectGeometry can have many vbo's, and wiring_ has information for all of them.
                  // We will use PerVboWiring as well. Multiple BufferObjectGeometry can use a single vbo.

  std::tr1::shared_ptr<FormattedIbo> ib_;

  // Internal struct for optimized vb binding order

  struct PerVbWiring { // here wre are within class BufferObjectGeometry
    ...
    //  The following map is from (attribute index in vb) --> relative index within BufferObjectGeometry's
    // vertex attributes:  This structure is needed, because multiple BufferObjectGeometry can use a single vbo.
  ...
  std::vector<PerVbWiring> perVbWirings_; 
    // a vector of PerVbWiring, which contains a vertex buffer object and the attribute indices associated with them.
  

  std::vector<std::string> vertexAttribNames_;

  // Setups up perVbWiring_ and vertexAttribNames_.
  // Gets called whenever wiringChanged_ is true
  // and we need to draw or return list of vertex attributes.
  void processWiring(); // Variables related to Vertex wiring are all private
...
struct VertexPNX : public VertexPN {
  ...
  VertexPNX& operator = (const GenericVertex& v) { // AHA! assignment operator is defined here so that the left hand side can take only
	                                               // subset of elements of right hand side GenericVertex.
    p = v.pos;
    n = v.normal;
    x = v.tex;
    return *this;
...
public:
  SimpleUnindexedGeometry(GLenum primType) : vbo ( new FormattedVbo( Vertex::FORMAT ) ) {

	    // Vertex is VertexPN, VertexPNX, or VertexPNBTX
	    //  VertexPN::FORMAT = VertexFormat(sizeof(VertexPN))
  ...
  SimpleUnindexedGeometry(const Vertex* vertices, int numVertices, GLenum primType)
  ...
  void upload(const Vertex* vertices, int numVertices) {
    vbo->upload(vertices, numVertices, true); // vbo = new FormattedVbo( Vertex::FORMAT )
  }
};
...
template<typename Vertex, typename Index> // Vertex = a generic vertex format
...
public:
  SimpleIndexedGeometry(GLenum primType )

    : vbo ( new FormattedVbo( Vertex::FORMAT ) ), ibo(new FormattedIbo(size2IboFmt(sizeof(Index)))) {
		//  VertexPN::FORMAT = VertexFormat( sizeof(VertexPN) ).put("aPosition", 3, GL_FLOAT, GL_FALSE, offsetof(VertexPN, p))
		//	     .put("aNormal", 3, GL_FLOAT, GL_FALSE, offsetof(VertexPN, n));
  ...
  // hierarchy:  Geometry -> BufferObjectGeometry -> SimpleIndexedGeometry

  SimpleIndexedGeometry(const string geoName, const Vertex* vertices,  const Index* indices, int numVertices, int numIndices, GLenum primType)

    : BufferObjectGeometry (geoName), vbo( new FormattedVbo(Vertex::FORMAT) ), ibo( new FormattedIbo(size2IboFmt(sizeof(Index))) ) {

    wireAttributes (vbo);
    indexedBy(ibo);
    //primitiveType(GL_TRIANGLES);
	primitiveType( primType );
	// bind the buffer and buffer data: The data pointed to by vertices is copied to the shader side.
	// It is not possible to access them on the CPU side, once SimpleIndexedGeometry is constructed.
    upload(vertices, indices, numVertices, numIndices); // glBindBuffer + glBindData
  }

  void upload(const Vertex* vertices, const Index* indices, int numVertices, int numIndices) {
    vbo->upload(vertices, numVertices, true); // bind the buffer and buffer data: The data pointed to by vertices is copied to the shader side 
