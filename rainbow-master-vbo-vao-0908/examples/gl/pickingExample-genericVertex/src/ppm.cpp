#include <cstdlib>
#include <cstring>
#include <cstdio>
#include <fstream>
#include <iostream>
#include <vector>
#include <string>
#include <stdexcept>

#include "ofMain.h"

//#include <QtGui/QOpenGLFunctions_3_0>
//extern QOpenGLFunctions_3_0 *m_funcs;

/*
#include <GL/glew.h>
#ifdef __MAC__
# include <GLUT/glut.h>
#else
# include <GL/glut.h>
#endif
*/
#include "ppm.h"

using namespace std;

extern std::ofstream messageFile;
std::ofstream phaseFile ("phaseFunctionRead.txt");
std::ofstream distanceFile ("distanceRead.txt");

extern GLint g_savedFramebuffer;

void printFBO(GLuint fbo, int width, int height, const char * filename)  {

	  vector<char> image(width*height*3);
	  
	// change the framebuffer to FBO from the default framebuffer

	glBindFramebuffer( GL_FRAMEBUFFER, fbo);  // a container for textures and an optional depth buffer
	//This is executed when drawing depending on the current mode: drawing or debug
	// The texture we're going to render to 

	
	// read the buffer
	glReadBuffer(GL_COLOR_ATTACHMENT0);
	glReadPixels(0, 0, width, height,  GL_RGB, GL_UNSIGNED_BYTE, &image[0]);

	ofstream f(filename, ios::binary);
    f << "P6 " << width << " " << height << " 255\n";
  
	for (int i = 0; i < height; ++i) {

       f.write( &image[ 3*width*(height-1-i) ], 3*width );
    }
	f.close();

	glBindFramebuffer(GL_FRAMEBUFFER, g_savedFramebuffer);
	//glBindFramebuffer(GL_FRAMEBUFFER, 0);
} // printFBO


void writePpmScreenshot(const int width, const int height, const char *filename) {
  vector<char> image(width*height*3);

  glReadPixels(0,0, width, height, GL_RGB, GL_UNSIGNED_BYTE, &image[0]);

  ofstream f(filename, ios::binary);
  f << "P6 " << width << " " << height << " 255\n";
  for (int i = 0; i < height; ++i) {

    f.write( &image[ 3*width*(height-1-i) ], 3*width );
  }
}

// Read one positive integer from a (text) file. Line beginning with
// "#" are ignored as comments.
static int ppmReadInteger(istream& is) {
  unsigned char ch;
  int got = 0, done = 0, accum = 0, inComment = 0;
  while (!done) {
    ch = is.get();

    if (inComment) {
      if (ch=='\n')
        inComment=0;
      continue;
    }

    if (isdigit(ch)) {
      accum = accum*10 + ch-'0';
      got = 1;
    }
    else if (ch=='#')
      inComment=1;
    else if (!ch || !strchr(" \t\r\n", ch))
      throw runtime_error("ppmRead: invalid character");
    else if (got)
      done = 1;
    else
      ; // nothing
  }
  return accum;
}

// Read the PPM header from the input stream and initialize the width and height
// to the appropriate values, and throws rumtime_error on invalid width/height
static void ppmReadHeader(istream& is, int &width, int &height) {
  if ((width = ppmReadInteger(is)) < 0) {
    throw runtime_error("ppmRead: invalid width");
  }
  if ((height = ppmReadInteger(is)) < 0) {
    throw runtime_error("ppmRead: invalid height");
  }
  if (ppmReadInteger(is) != 255) {
    cerr << "Warning: maxcolor not 255 : won't work well" << endl;
  }
}

//Reads the actual PPM data and stores returns in in a pixels.
void ppmRead(const char *filename, int& width, int& height, std::vector<PackedPixel>& pixels) {
  ifstream is(filename, ios::binary);
  if (!is.is_open())
    throw runtime_error(  std::string( "ppmRead: Cannot open file " ) + filename + " for read"  );

  // Sets bits to report IO error using exception
  is.exceptions(ios::eofbit | ios::failbit | ios::badbit);

  char buf[2];
  is.read(buf, 2);

  bool isbinary = false;
  if (!memcmp(buf, "P3", 2))
    isbinary = false;
  else if (!memcmp(buf, "P6", 2))
    isbinary = true;
  else
    throw runtime_error("ppmRead: bad file format");

  ppmReadHeader(is, width, height);

  pixels.resize(width * height);

  if (isbinary) {
    for (int row = height - 1; row >= 0; row--) {
      is.read(reinterpret_cast<char*>(&pixels[row * width]), width * sizeof(PackedPixel));
    }
  }
  else {
    for (int row = height - 1; row >= 0; row--) {
      for (int l = 0; l < width; l++) {
        PackedPixel &p = pixels[row * width + l];
        p.r = ppmReadInteger(is);
        p.g = ppmReadInteger(is);
        p.b = ppmReadInteger(is);
      }
    }
  }
}



// Read one positive integer from a (text) file. Line beginning with
// "#" are ignored as comments.
static int readInteger(istream& is) {
  unsigned char ch;
  int got = 0, done = 0, accum = 0, inComment = 0;

  while (!done) {
    ch = is.get();

    if (inComment) {
      if (ch=='\n')
        inComment=0;
      continue;
    }

    if (isdigit(ch)) {
      accum = accum*10 + ch-'0';
      got = 1;
    }
    else if (ch=='#')
      inComment=1;
    else if (!ch || !strchr(" \t\r\n", ch))
      throw runtime_error("ppmRead: invalid character");
    else if (got) 
      done = 1;
    else
      ; // nothing
  }
  return accum;
} // readInteger

void read3DFloatTex( const char *filename, int& depth, int& height, int& width, 
					 vector<float>& distance) {
	ifstream is (filename);


	if ( !is.is_open() )
		throw runtime_error(std::string("read 3DFloatTex: Cannot open file ") + filename + " for read");
	
	// Sets bits to report IO error using exception
	is.exceptions(ios::eofbit | ios::failbit | ios::badbit);

	// read header
	if (( depth  = readInteger(is) ) < 0) {
		throw runtime_error("read 3DFloatTex: invalid width");
	}
	if (( height  = readInteger(is) ) < 0) {
		throw runtime_error("read 3DFloatTex: invalid height");
	}
	if ((  width  = readInteger(is) ) < 0) {
		throw runtime_error("read 3DFloatTex: invalid height");
	}

	distance.resize( width * height * depth );

	distanceFile << "read3DDistanceTex" << endl;

	int s, t, r;
	for (r = 0; r < depth; r++) {
	
		for (t = 0; t < height; t++) {
			for (s = 0; s < width; s++) {
			
				// is >> d; is >>d: is >> d; the data may differ in type case by case; 
				// read them as char strings deliminated by deliminators
				
               float d;
				// read indices
		       							 
				is >> d; is >> d; is >> d;
			
			   // read data
			
			    is >> d;

			    distance[ r* height * width + t * width + s ] = d;

			    distanceFile << "(" << s << "," << t << "," << r << ")=" << d << endl;


				
			}
		}
	}
	
	distanceFile.close();

	/*

	for (unsigned int i = 0; i < distance.size(); i++)
	{
		float d;
		//PackedDistance d_in_rgb;
		// discard first three digits
		
		is >> d; is >> d; is >> d;

		//get distance value
		// istream& operator>> (int& val);
         // istream& operator>> (float& val);
         

		is >> d;

		phaseFile << d <<  endl;
				

		distance[i] = d;
		
	}
	*/
}



void read1DFloatTex(const char *filename, int& width,   vector<float>& floatData) {
	
	ifstream is (filename);


	if (!is.is_open())
		throw runtime_error(std::string("1DFloatTex Read: Cannot open file ") + filename + " for read");
	
	// Sets bits to report IO error using exception
	is.exceptions(ios::eofbit | ios::failbit | ios::badbit);

	// read header
	if ((width = readInteger(is)) < 0) {
		throw runtime_error("read 1DFloatTex: invalid width");
	}


	floatData.resize( width);

	for (unsigned int i = 0; i < floatData.size(); i++)
	{
		float d;
		
		// discard first number (which is a  key )

		
		//readInteger(is);
		
		is >> d;


		//get the data
		
		is >> d;
		//out << d << endl;	// to debug
			
		floatData[i] = d;
		
	}

	
}

void read2DFloatTex(const char * filename, int& height, int& width, vector<float>& floatData) {

	ifstream is (filename);


	if (!is.is_open())
		throw runtime_error( std::string("2DFloatTex Read: Cannot open file ") + filename + " for read");
	
	// Sets bits to report IO error using exception
	is.exceptions(ios::eofbit | ios::failbit | ios::badbit);

	// read header
	if (( height  = readInteger(is)) < 0) {
		throw runtime_error("read 2DFloatTex: invalid width");
	}
	if (( width  = readInteger(is)) < 0) {
		throw runtime_error("read 2DFloatTex: invalid height");
	}

	floatData.resize( width * height);

	messageFile << "read2DFloatTex" << endl;

	for (unsigned int i = 0; i < floatData.size(); i++)
	{
		float d;
		
		// discard first two  digits (which are key )

		
		//readInteger(is); 
		//readInteger(is);

		is >> d; is >> d;
	
		//get the data
		
		is >> d;
		//out << d << endl;	// to debug
			
		floatData[i] = d;
		//messageFile << d << endl;
	}

	
}
