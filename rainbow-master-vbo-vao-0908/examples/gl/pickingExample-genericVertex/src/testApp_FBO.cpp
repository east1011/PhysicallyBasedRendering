...
void testApp::drawForPicking() {
  ...
  glClearColor(1, 1, 1, 1); // this WHITE background color is only meant to be used
...
// Declare an empty uniforms
 ...
 // extraUniforms.put("uSunRayDir", g_sunRayDir );
  ...
  if ( !picking ) { // not picking mode but an ordinary drawing mode to FBO
	...
	// framebuffer object with value zero is reserved to represent the default framebuffer provided by the windowing system.
    // So, use a nonzero framebuffer object
     GLuint fbo;
	...
	// When the fragment shader writes a value to the output variable 0, it will be 
	// written to GL_COLOR_ATTACHMENT0 of FBO, and when it writes a value to the output variable 1, 
	// it will be written to GL_DEPTH_ATTACHMENT of FBO. 
   ...
   for ( int i  = 0; i < g_objectList.size() - 1; i++ ) { // (5) This loop goes over the list of objects g_objectList and draw each object in the list.
   ...
   // draw rainbow object "AABBrainbow": use the color texture and the depth texture attached to FBO,
   // send them to the shader for writing to the system provided buffer.



    int rainbowIndex = g_objectList.size() - 1;
	...
	// bind the colorTex and depthTex of FBO to the rainbow shader
	// g_rainbowMat is the material shader of the rainbow object, which is referred to in g_objectList[rainbowIndex]
	// Change its properties, so that they are used when rendering

	g_rainbowMat->getUniforms().put("uColorTex", shared_ptr<ShaderImageTexture2D_RGB_RGB>(new ShaderImageTexture2D_RGB_RGB("sourceimages/sourceimages/Fieldstone.ppm", true)));
	g_rainbowMat->getUniforms().put("uDepthTex", shared_ptr<ShaderImageTexture2D_RGB_RGB>(new ShaderImageTexture2D_RGB_RGB("sourceimages/sourceimages/FieldstoneNormal.ppm", false)));


	// draw rainbow
	 try {
      g_objectList[rainbowIndex]->draw( extraUniforms ); 
	  // This draw leads to  BufferedObjectGeometry::draw() which binds vertex buffers 
	  // and set vertex     attribute pointers, and then calls drawElements() */
  ...
  else { // picking mode: do not draw a rainbow => no need to use the depth of the scene => use the system framebuffer
     // 
...
// a) expose renderable pixelformats that can't be used as texture formats (for instance, multisampled renderbuffers
//	 in the days when ARB_texture_multisample was not born yet)
// b) since the driver knows that a renderbuffer can't be ever used for a texture, 
//it has the opportunity to chose a different, more render-optimized memory layout for the renderbuffer. 

// So, my advice is to use renderbuffers instead of textures whenever you can 
//	 (i.e. don't need to sample the rendered data in a shader afterwards).

// The only way of using the result produced with render buffer is to copy rectangular data 
//	to the displayable framebuffer?
...
// But when one use Textures it's is possible to detach the texture and use it just the way you like. 
// I guess render buffers can be more optimized for some operations rather than textures. 
// I don't see any other advantage then performance 
//(I'm not saying this is not an important issue)
   ...
   // g_overridingMaterial = g_pickingMat;
    ...
    //Matrix4 NMVM = normalMatrix(MVM);
	 ...
	 if ( g_objectList[i]->objectName == "AABBrainbow" ) {
       ...
       // AABBmin and AABBmax with respect to camera space: 
	   ...
	   extraUniforms.put("uAABBmin", AABBmin );   
	   extraUniforms.put("uAABBmax", AABBmax );  
	   */
...
void testApp::initGLState() {
	...
	if (!g_Gl2Compatible) 
		...
		glEnable(GL_FRAMEBUFFER_SRGB); // For opengl3.x,  request an sRGB frame buffer using the call glEnable(GL FRAMEBUFFER SRGB).
                                       // Then we can pass linear [R, G, B] values out from the fragment shader. 
	                                   //  and they will be gamma corrected into the sRGB format before begin sent to the screen.
...
// 1: create a ground
  ...
  */
  ...
  shared_ptr<Geometry> groundGeometry  ( new SimpleIndexedGeometryPNTBX( &vtxGround[0], &idxGround[0], vbLen, ibLen, GL_TRIANGLES  )  );
...
//	  .put("aNormal", 3, GL_FLOAT, GL_FALSE, offsetof(VertexPN, n));
 ...
 shared_ptr<Geometry> cubeGeometry1 (  new SimpleIndexedGeometryPNTBX( &vtxCube1[0], &idxCube1[0], vbLen, ibLen, GL_TRIANGLES ) ); // reset(T * p)
 ...
 shared_ptr<Geometry> cubeGeometry2 (  new SimpleIndexedGeometryPNTBX( &vtxCube2[0], &idxCube2[0], vbLen, ibLen, GL_TRIANGLES ) ); // reset(T * p)
...
//messageFile << "invModelMatrixAABB:" << invModelMatrixAABB << endl;
 ...
 shared_ptr<Geometry> AABBGeometry (  new SimpleIndexedGeometryPNTBX( &vtxAABB[0], &idxAABB[0], vbLen, ibLen, GL_QUADS ) ); 
...
//		                       Matrix4::makeTranslation( 	Cvec3( 0, g_AABBsize[1]/2 + 6.4, -g_AABBsize[2]/2.0 ) ) ) );
  ...
  id= 0 ; // set the rainbow AABB box id to zero. // AABB box can be picked and moved.
  id = id << 8;  
  id = id | 255; // 
  pickColor  = g_picker.idToColor(id );
...
//messageFile << "invModelMatrixAABB:" << invModelMatrixAABB << endl;
 ...
 shared_ptr<Geometry> AABBGeometry (  new SimpleIndexedGeometryPNTBX( &vtxAABB[0], &idxAABB[0], vbLen, ibLen, GL_QUADS ) ); 
  ...
  g_rainbowMat->getUniforms().put("uPhaseTex", shared_ptr<ShaderImageTexture3D_RF_RF>(new ShaderImageTexture3D_RF_RF("rainbow/phaseFunction2_120_100_fort.txt")));
   ...
   // g_AABBRbt is already set when AABB is created.

  //g_AABBRbt.reset(  new SgRbtNode (  g_SceneRbt * 
//		                       Matrix4::makeTranslation( 	Cvec3( 0, g_AABBsize[1]/2 + 6.4, -2.5) ) ) );
  g_AABBRbt.reset(  new SgRbtNode (  g_SceneRbt * 
		                       Matrix4::makeTranslation( 	Cvec3( 0, g_AABBsize[1]/2, -g_AABBsize[2]/2.0) ) ) );

  shared_ptr<Object> AABBrainbow ( new Object( "AABBrainbow",  g_AABBRbt, AABBGeometry, g_rainbowMat) );
 
  g_objectList.push_back( AABBrainbow );
