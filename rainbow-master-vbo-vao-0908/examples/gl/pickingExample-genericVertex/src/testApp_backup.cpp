...
// set the parameters for the camera, which define the internal working of the camera.
    g_frustMinFov = 45.0;  // A minimal of 100 degree field of view for rainbow viewing
	...
	g_SceneRbt = Matrix4::makeAxisRotation( - PI / 2, Cvec3(0,1,0) ); 
   ...
   for ( int i  = 0; i < g_objectList.size(); i++ ) { // (5) This loop goes over the list of objects g_objectList and draw each object in the list.
    ...
    //Matrix4 NMVM = normalMatrix(MVM);
	 ...
	 if ( g_objectList[i]->objectName == "AABBrainbow" ) {
       ...
       /*
	   Cvec3 AABBmin = Cvec3(  uInvModelMatrixAABB * Cvec4( g_AABBmin, 1.0) );
	   Cvec3 AABBmax =  Cvec3( uInvModelMatrixAABB  * Cvec4( g_AABBmax, 1.0) );
	   */
	   /*
	   ...
	   */
       ...
       Cvec3 uAABBmin = Cvec3(  g_invEyeRbt * Cvec4( g_AABBmin, 1.0) );
       Cvec3 uAABBmax =  Cvec3( g_invEyeRbt * Cvec4( g_AABBmax, 1.0) );
 

	   extraUniforms.put("uAABBmin", uAABBmin  );   
	   extraUniforms.put("uAABBmax", uAABBmax );   
	 }
