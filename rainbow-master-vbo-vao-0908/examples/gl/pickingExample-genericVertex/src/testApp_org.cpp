...
extern int g_argc;
extern char **g_argv;  // defined in main.cpp
...
double  g_frustMinFov = 170.0;  // A minimal of 100 degree field of view for rainbow viewing

double  g_frustFovY = g_frustMinFov; // FOV in y direction (updated by updateFrustFovY)

double  g_frustNear = 100.0;    // near plane
double  g_frustFar = -2000.0;    // far plane
...
//float g_phaseFunction[ nRadii * nSpectrum * nThetas ];
...
static Cvec3 g_AABBmin, g_AABBmax;
...
vector < shared_ptr<Object> >  g_objectList;  // This object list is filled by GPURender function api.cpp

std::ofstream messageFile("./messageFile.txt", std::ios::out);
std::ofstream phaseFile("./phaseFunctionDebug.txt", std::ios::out);
...
spa_data spa;  //declare the SPA structure
	...
	messageFile  << "sunRay in geocentric coord = (" << sunRay  << endl;
...
// convert the sun light to its RGB representation
    ...
    */
	...
	for (lambda = lambdaStart, j=0; j < nLambdas; lambda += lambdaStep, j++ ) {
	 // integration of cie-color-match curve over  nSpectrumteps intervals,
...
// set the parameters for the camera, which define the internal working of the camera.
	...
	if ( norm2( negZAxis - groundCamDir ) < 1.0e-8 ) { // no need to rotate; The rotation matrix will be identity
        ...
        g_cameraRotMat = Matrix4::makeAxisRotation( theta, normalize(rotAxis) );
...
void testApp::setupFloatTexturesForVolume() {

   // add volume textures to each shader; it is assumed that each shader is able to handle
	// the volume textures.
	
	//g_rainbowMat->getUniforms().put("uDistTex", shared_ptr<ShaderImageTexture3D>(new ShaderImageTexture3D("distancefield/distancefield.txt", true, true)));

	//g_rainRainbowMat->getUniforms().put("uScatTex", shared_ptr<ShaderImageTexture2D_RF_RF>(new ShaderImageTexture2D_RF_RF("rainbow/scattering2_120_fort.txt", false)));

	//g_rainRainbowMat->getUniforms().put("uPhaseTex", shared_ptr<ShaderImageTexture3D_RF_RF>(new ShaderImageTexture3D_RF_RF("rainbow/phaseFunction2_120_100_fort.txt",  false)));
    ...
    */
	...
	// File "distancefield/distancefield.txt" is relative to the working directory where the project file is located.

	/* 
	g_redDiffuseMat->getUniforms().put("uDistTex", shared_ptr<ShaderImageTexture3D>(new ShaderImageTexture3D("distancefield/distancefield.txt", true)));
	g_redDiffuseMat->getUniforms().put("uScatTex", shared_ptr<ShaderImageTexture2Df>(new ShaderImageTexture2Df("rainbow/scattering2_120.txt", true)));

	g_redDiffuseMat->getUniforms().put("uPhaseTex", shared_ptr<ShaderImageTexture3D>(new ShaderImageTexture3D("rainbow/phaseFunction2_120_100.txt", true)));

	
	g_blueDiffuseMat->getUniforms().put("uDistTex", shared_ptr<ShaderImageTexture3D>(new ShaderImageTexture3D("distancefield/distancefield.txt", true)));
	g_blueDiffuseMat->getUniforms().put("uScatTex", shared_ptr<ShaderImageTexture2Df>(new ShaderImageTexture2Df("rainbow/scattering2_120.txt", true)));

	g_blueDiffuseMat->getUniforms().put("uPhaseTex", shared_ptr<ShaderImageTexture3D>(new ShaderImageTexture3D("rainbow/phaseFunction2_120_100.txt", true)));

	
	g_greenDiffuseMat->getUniforms().put("uDistTex", shared_ptr<ShaderImageTexture3D>(new ShaderImageTexture3D("distancefield/distancefield.txt", true)));
	g_greenDiffuseMat->getUniforms().put("uScatTex", shared_ptr<ShaderImageTexture2Df>(new ShaderImageTexture2Df("rainbow/scattering2_120.txt", true)));

	g_greenDiffuseMat->getUniforms().put("uPhaseTex", shared_ptr<ShaderImageTexture3D>(new ShaderImageTexture3D("rainbow/phaseFunction2_120_100.txt", true)));
	*/
...
void testApp::draw()  { // callback function for draw event
	...
	//g_windowHeight = (float) nSpectrum; 
...
//  extraUniforms.put("uInvEyeMatrix", invEyeRbt);
  ...
  extraUniforms.put("uLightRGBColor", g_lightRGBColor); // lightColor for rainbow
...
// i < 12
    ...
    //Matrix4 NMVM = normalMatrix(MVM);
	 ...
	 if ( g_objectList[i]->objectName == "AABBrainbow" ) {
       ...
       extraUniforms.put("uEyeMatrix", g_eyeRbt); 
  ...
  // use  Uniforms& put(const std::string& name, const Cvec<float, n> *v, int count)
     //  extraUniforms.put("uModelMatrixAABB", g_modelMatrixAABB);

      // extraUniforms.put("uPhaseFunction", g_phaseFunction, nRadii* nSpectrum * nThetas); 

	   
	   messageFile << "AABBRbt =\n" << *g_AABBRbt << endl;

       Cvec3 newAABBmin = Cvec3( *g_AABBRbt * Cvec4( g_localAABBmin, 1) );

	   Cvec3 newAABBmax = Cvec3 ( *g_AABBRbt * Cvec4 ( g_localAABBmax, 1) );
	   
	   messageFile << "newAABBmin \n" << newAABBmin << endl;
	   messageFile << "newAABBmax \n" << newAABBmax << endl;


	   extraUniforms.put("uAABBmin", Cvec3( g_invEyeRbt * Cvec4( newAABBmin,1) ) );   
	   extraUniforms.put("uAABBmax", Cvec3( g_invEyeRbt * Cvec4( newAABBmax,1) ) );   
       extraUniforms.put("uInvEyeMatrix", g_invEyeRbt);
	...
	// g_overridingMaterial-> getUniforms().put("uMaterialColor", pickColor);
	 ...
	 if ( g_objectList[i]->objectName == "AABBrainbow" ) { // for debugging, only rainbow
 ...
 int   d_windowWidth =  nThetas;  
 int   d_windowHeight = nLambdas; 
...
//  extraUniforms.put("uInvEyeMatrix", invEyeRbt);
  ...
  extraUniforms.put("uLightRGBColor", g_lightRGBColor);
	...
	// Poor filtering. Needed ! 
	    ...
	    glTexImage2D(GL_TEXTURE_2D, 0,GL_R32F, d_windowWidth, d_windowHeight, 0,GL_RED,
		         GL_FLOAT, NULL);   
 ...
 // glReadPixels(x,y, width, height, GL_RGBA, GL_UNSIGNED_BYTE, &pixel[0]);
	...
	for (int j = 0; j < d_windowHeight; j++ )
	 ...
	 {
		   glReadPixels(i,j, 1,1, GL_RED, GL_FLOAT, &data[0]);

		  // phaseFile << "pixel: " << "(" << lambdaStart + i * lambdaStep  << "," << thetaStart + j *thetaStep << ") = " << data[i * d_windowHeight + j] << endl; 
		  // cout << "pixel: " << "(" <<  lambdaStart + i *lambdaStep << "," << thetaStart + j * thetaStep << ") = " << data[i * d_windowHeight + j] << endl; 
		   phaseFile << "pixel: " << "(" <<  i   << "," <<  j  << ") = " << data[0] << endl; 
		   cout << "pixel: " << "(" <<   i  << "," <<  j  << ") = " << data[0] << endl; 
...
void testApp::initGLState() {
	...
	checkGlErrors("after glColorMask() ");
	...
	checkGlErrors("after glBindFrameBuffer() ");

	GLuint attachments[1] = { GL_BACK_LEFT }; 
	glDrawBuffers(1,  &attachments[0] );  // ONLY GL_NONE, GL_FRONT_LEFT, GL_BACK_LEFT, GL_BACK_RIGHT are accepted

	checkGlErrors("after glDrawBuffers(1, attachments) ");  // INVALID_OPERATION error ocurred here, because I used GL_BACK
	...
	glReadBuffer(GL_FRONT); // or GL_FRONT_LEFT;  default, the same with glDrawBuffer()
	checkGlErrors("after glReadBuffer(GL_FRONT) ");

	if (!g_Gl2Compatible) {
		glEnable(GL_FRAMEBUFFER_SRGB); // For opengl3.x, enable the sRGB framebuffer update and blending capability
		checkGlErrors("after glEnable(GL_FRAMEBUFFER_SRGB) ");
	}
	...
	try {
		...
		std::cout << error.what() << endl;
		messageFile << error.what() << endl;
		//throw; // "GL_INVALID_OPERATION" error  occurs within initGLState(); The parameters of the function are invalid, or
		// the current operation cannot be performed in the current context. etc... Some invalid operation may be ignored,
...
/*
	try {
	  ...
	  g_bumpFloorMat->getUniforms().put("uTexUnit0", shared_ptr<ShaderImageTexture2D>(new ShaderImageTexture2D("image/Fieldstone.ppm", true)));
	//g_bumpFloorMat->getUniforms().put("uTexNormal", shared_ptr<ShaderImageTexture2D>(new ShaderImageTexture2D("image/FieldstoneNormal.ppm", false)));
...
//messageFile << "invModelMatrixAABB:" << invModelMatrixAABB << endl;
 ...
 // NOTE: here, vtxCube and idxCube store the array data for  vbo and ibo; When they are bound to the vbo and ibo, they are
 // copied to the safe area; It is OK to destroy the data after the binding.  
...
//  shared_ptr<SgRbtNode> groundRbt ( &SgRbtNode::makeTranslation( Cvec3(0,0,0) ) ); 
 // shared_ptr<SgRbtNode>  cubeRbt ( &SgRbtNode::makeTranslation( Cvec3(5.0,0,0)  ) );
  ...
  //shared_ptr<SgRbtNode> AABBRbt ( new SgRbtNode( g_modelMatrixAABB ) );
  
 // shared_ptr<SgRbtNode> g_AABBRbt  ( new SgRbtNode (SgRbtNode::makeTranslation( g_AABBcenter) ) );
  
  Cvec3 localTranslation = Cvec3 (g_invCameraRotMat * Cvec4( g_initAABBcenter, 0) );
 
  g_AABBRbt.reset( new SgRbtNode ( g_cameraRotMat * SgRbtNode::makeTranslation( localTranslation) ) );


 
 // shared_ptr<SgRbtNode> g_AABBRbt  
 //	        ( new SgRbtNode ( SgRbtNode::makeTranslation( localTranslation) ) );
  
  messageFile << "g_AABBRbt 1 =" <<  Matrix4::makeTranslation( g_initAABBcenter) * g_cameraRotMat
	          << "g_AABBRbt 2 =" << *g_AABBRbt << endl;
  ...
  shared_ptr<Object> cube ( new Object( "cube", cubeRbt, cubeGeometry, g_blueDiffuseMat) );
  shared_ptr<Object> sphere ( new Object("sphere", sphereRbt, sphereGeometry, g_redDiffuseMat) );
  ...
  // setup the textures for scattering and phase function.

  //setupFloatTexturesForVolume();
  ...
  */
  ...
  // Insert only AABB box into the global object list
