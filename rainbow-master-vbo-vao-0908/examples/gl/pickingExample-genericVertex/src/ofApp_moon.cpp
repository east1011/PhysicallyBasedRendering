...
shared_ptr<Object> g_AABBRainbowAndSceneMatObj, g_oldAABBRainbowOnlyMatObj;;

Matrix4 g_projectionMatrix; 

int  g_objId = 0;  // g_objId ==0 refers to AABB
...
SgRbtNode g_SceneRbt;  // SgRbtNode = Matrix4
...
void ofApp::setup(){
	...
	names.push_back("render AABBRainbow");
	...
	*/
	...
	cout << "init window size= (" << g_windowWidth << "," << g_windowHeight << ")" << endl;
	// 1024,768 = default
	...
	//initObjects(); // initObjects() is for boxes and ground. objId of these objects continue from the current id
	               // set by initPBRTObjects()
	initAABB(); // objId of AABB =0
...
// set the parameters for the camera, which define the internal working of the camera.
    g_frustMinFov = 80.0;  // old: A minimal of 100 degree field of view for rainbow viewing
...
//  The distance between n and f should be short as possible to minimize the depth buffer precision problem.
	...
	g_SceneRbt = Matrix4::makeAxisRotation( - PI / 5.0, Cvec3(0,1,0) ); //for NationalModernArt
	...
	//float distanceToEye =  2;  // the observer on the ground in Asian Game
	float distanceToEye =  47;  // the observer on the ground in Asian Game
	Cvec3 eyeLocation = Cvec3(0, eyeHeight, distanceToEye);

	// the position of the eye relative to the world coordinate system.
	// set the camera rotation so that it looks at the scene with the sun right behind
...
Matrix4 ofApp::makeProjectionMatrix() {
	...
	return Matrix4::makeProjection( g_frustFovY, aspectRatio, g_frustNear, g_frustFar);
}
...
void ofApp::myOwnDraw() {
    ...
    if ( g_renderMode == 4 ) { // AABBrainbow  
		cout << "renderAABBRainbowOnlyToScreen():"  << endl;
	   try {
       	renderAABBRainbowAndSceneToScreen();
...
void ofApp::renderAABBRainbowAndSceneToScreen() {
   ...
   extraUniforms.put("uDropDensity", g_dropDensity );  
	...
	// draw rainbow to the default buffer
	 try {
      ...
      Matrix4 MVM = g_invEyeRbt * (  *( g_AABBRainbowAndSceneMatObj -> objectRbt ) ) ; // g_currentPickedObject->objectRbt may have been
	                                                              // changed by picking
  
	  extraUniforms.put("uModelViewMatrix", MVM).put("uNormalMatrix", normalMatrix(MVM) );
   
      g_AABBRainbowAndSceneMatObj->draw( extraUniforms ); 
...
void ofApp::drawBackgroundStuff()  {
 ...
 // extraUniforms.put("uSunRayDir", g_sunRayDir );
  ...
  // map the NDC to the window viewport
  glViewport(0, 0, g_windowWidth, g_windowHeight);
   ...
   for ( int i  = 0; i < g_objectPtrList.size(); i++ ) { // (5) This loop goes over the list of objects g_objectPtrList and draw each object in the list.
     ...
     //cout << "Frame of" << "object " << i << *(g_objectPtrList[i]->objectRbt) << endl;
...
void ofApp::initMaterials() {
	...
	g_arcballMat->getUniforms().put("uMaterialColor", Cvec4f(1.0f, 1.0f, 1.0f, 1.0));
...
// 1: create a ground
    ...
    static const float groundSize = 1030000.0;   // half the ground length
...
// for National Modern Musium
 //	g_AABBRbt.reset(  new Matrix4 (   g_cameraRotMat * 
...
// MaterialShader rainbowOnlyMat ("rainbow+rainbow", "shaders/rainbow-gl3.vshader", "shaders/rainbow-only-gl3.fshader");
 MaterialShader rainbowAndSceneMat ("rainbow+rainbow", "shaders/rainbow-gl3.vshader", "shaders/rainbow-RGB-STAN-gl3.frag");
 g_AABBRainbowAndSceneMat.reset(new MaterialShader( rainbowAndSceneMat) );

 g_AABBRainbowAndSceneMat->getUniforms().put("uScatTex", shared_ptr<ShaderImageTexture2D_RF_RF>(new ShaderImageTexture2D_RF_RF("rainbow/scattering2_120_fort.txt")));

 g_AABBRainbowAndSceneMat->getUniforms().put("uPhaseTex", shared_ptr<ShaderImageTexture3D_RF_RF>(new ShaderImageTexture3D_RF_RF("rainbow/phaseFunction2_120_100_fort.txt")));

 g_AABBRainbowAndSceneMatObj.reset ( new Object( "AABBRainbow",  g_AABBRbt, AABBGeometry, g_AABBRainbowAndSceneMat) );
    ...
    else if(name == "DROPDOWN")
    {
		...
		for(int i = 0; i < selected.size(); i++) {  
	     ...
	     if ( action_name == "render Scene") {
		  ...
		  else if ( action_name == "render AABBRainbow") { // render to FBO
			 checkGlErrors();
		     renderAABBRainbowAndSceneToScreen();
			 checkGlErrors();

			 g_renderMode = 4; // AABBrainbow
...
void ofApp::initGLState() {
	...
	glEnable(GL_DEPTH_TEST);
	//glDisable(GL_DEPTH_TEST); // this is default
	glClearDepth(0.);
