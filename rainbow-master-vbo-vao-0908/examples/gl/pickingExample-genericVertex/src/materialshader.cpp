...
struct GlProgramDesc {
	...
	GlProgramDesc(GLuint vsHandle, GLuint fsHandle, string vsFilename, string fsFilename)  {
		...
		try {
			...
			messageFile << "link shader: " << error.what() << endl;
		...
		if (!g_Gl2Compatible) {
			...
			cout << "maxDrawBuffers=" << maxDrawBuffers << endl;

			glBindFragDataLocation(program, 0, "fragColor");
			glBindFragDataLocation(program, 1, "spect1");
			glBindFragDataLocation(program, 2, "spect2");
			glBindFragDataLocation(program, 3, "spect3");
			glBindFragDataLocation(program, 4, "spect4");
			glBindFragDataLocation(program, 5, "spect5");
			glBindFragDataLocation(program, 6, "spect6");
			glBindFragDataLocation(program, 7, "spect7");		
		}
...
void MaterialShader::draw(Geometry& geometry, const Uniforms& extraUniforms) {
	...
	//std::cout << "geometry=" << geometry.geoName << endl;
	...
	//cout << "geometry=" << geometry.geoName << endl;
	...
	int maxLength;
	glGetProgramiv(programDesc_->program, GL_INFO_LOG_LENGTH, &maxLength);

	//The maxLength includes the NULL character

	//The maxLength includes the NULL character
	GLint * errorLog = new GLint[maxLength];
	...
	for (int i = 0, n = programDesc_->uniforms.size(); i < n; ++i) { 
		...
		for (; j < 2; ++j) {  // uniformsList[0] = uniforms_, and uniformsList[1] = extraUniforms
			...
			if (userUniformValue == NULL && shaderUniform.name.length() >= 3 
				&& shaderUniform.name.compare( shaderUniform.name.length() - 3,  3,  "[0]") == 0)	// compare(pos, size, str)
	...
	*/
	...
	for (int i = 0, n = programDesc_->attribs.size(); i < n; ++i) {
		...
		if (j == numAttribs) {
			stringstream s;
			s << shaderName << ": " << string("Vertex attribute ") + ad.name + ": used in the shader codes, but not supplied." ;
			throw runtime_error(s.str() );
