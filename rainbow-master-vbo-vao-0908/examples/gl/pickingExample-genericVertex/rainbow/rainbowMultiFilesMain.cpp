...
// The above is allowed because "../", the folder above the source folder is declared
// as the include folder. Core include files can be included without mentioning
// "core" folder, because "../core" is included as an include file
...
double phasefunction(double lambda, double radius, double theta, double FCscat);
	 ...
	 dcomplex &rp, dcomplex &rv, dcomplex &t, dcomplex &r, double &u, double &v )
{
    ...
    rp = dcomplex( (re*re - im*im) * cos(in_angle) - u, 2. * re * im * cos(in_angle) - v) / 
		 dcomplex ( (re*re
		 - im*im) * cos(in_angle) + u, 2. * re * im * cos(in_angle) + v);
...
//    const int nRadii = 50;
    ...
    for ( int i =0; i < nRadii; i++ ) {
        ...
        for (int j=0; j < nSpectralSamples; j ++ ) {
            ...
            file << std::setw(15) << std::setprecision(6) << lambda  << std::setw(15) << Cabs[ i ][ j ] << std::endl;
    ...
    for (int i=0; i< nRadii; i++ ) {
		...
		for (int j =0; j < nSpectralSamples; j++ )  {
            ...
            file << std::setw(15) << std::setprecision(6) << lambda  << std::setw(15) << FCscat[ i ][ j ] << std::endl;
