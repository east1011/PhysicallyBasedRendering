 ...
 */
double particlePhaseFunction(double lambda, double radius, double theta, double FCscat)
{
...
//    const int nRadii = 50;
	...
	fileName = "absorption30_120.txt";

	file.open( fileName.c_str() );

    file << std::setiosflags (std::ios::scientific );

	file  << "radius[mm]" << std::setw(15) <<  "wavelength[nm]" <<  std::setw(15) << "absCrossSection" << std::endl;
	file << nRadii << " " <<  nSpectralSamples <<  std::endl;

	/*
    for ( int i =0; i < nRadii; i++ ) {

		// string fileName = "C:/Users/moon/pbrt-v2-src/pbrt-v2/pbrt-scenes/rainbow/absorption/absradius";
    ...
    //    file << std::setw(15) <<  "wavelength[m]" << std::setw(15) << "Cabs" << std::endl;
        ...
        for (int j=0; j < nSpectralSamples; j ++ ) {
            ...
            file <<  std::setprecision(6) << radius << std::setw(15) << lambdanm  << std::setw(15) << Cabs[ i ][ j ] << std::endl;
    ...
    for (int i=0; i< nRadii; i++ ) {
      ...
      //  file <<  std::setw(15) << "wavelength[m]" << std::setw(15) << "Cabs" << std::endl;
		...
		for (int j =0; j < nSpectralSamples; j++ )  {
			...
			file <<  std::setprecision(6) << radius << std::setw(15) << lambdanm  << std::setw(15) << FCscat[ i ][ j ] << std::endl;
...
*/
	...
	const int nThetas = 100; // 1/2 degree in each step between theta 130 deg and theta 180;

	double theta;
	 
	const  double pi = acos( -1.0e0);  // -1.0 const double in default; -1.0f = float

	double thetaStart = 130.0 * pi / 180.0;
	double thetaEnd = pi;
	...
	fileName = "phaseFunction30_120_100.txt";
    file.open( fileName.c_str() );
    ...
    for ( int i=0; i  < nRadii; i++ ) {
        ...
        for (int j=0; j < nSpectralSamples ; j++ ) {
		   ...
		   /* string folder  = "C:/Users/moon/pbrt-v2-src/pbrt-v2/pbrt-scenes/rainbow/phase/";
		   
		   string fileName ="Radius";
		   fileName += to_string( i+1);
		   fileName += "Lambda";
		   ...
		   */
		   ...
		   for (int k =0; k < nThetas; k++ ) {
              ...
              //  file << std::setw(15) << std::setprecision(3) << radius * 1.0e+3 << std::setw(20)

              //  << std::setprecision(4) << lambda * 1.0e+9  << std::setw(15) << theta * 180./pi <<
              //  std::setw(15) << phase[ i ][ j ] [ k ]  << std::endl;
                 
				file << std::setw(15) << std::setprecision( 6) << radius << std::setw(15) << lambdanm << std::setw(15) <<  
