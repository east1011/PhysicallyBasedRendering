...
//	ofLogNotice("ofAppGLFWWindow") << "WINDOW MODE IS " << screenMode;
	...
	if(glVersionMinor!=-1 && glVersionMajor!=-1){
		...
		if(glVersionMajor>=3){
			//glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE); => access violation to glCreateShader() because of NULL function pointer
		...
		#endif
	} // this block is not executed when GLProgrammableShader is not
	   // set, so that CORE_PROFILE is not used. 
...
void ofAppGLFWWindow::runAppViaInfiniteLoop(ofBaseApp * appPtr){
	...
	ofNotifySetup(); // setup is defined in ofApp.cpp in openframeworks
	...
	while(!glfwWindowShouldClose(windowP))  {
	   ...
	   // ofNotifyDraw(); // process ALL draw events waiting in the queue, including 
	                  // draw events for the GUI, by notifying them to the associated listener methods.
	
		//if (g_redrawWindowEvent) {
		 // to draw the scene I created by using my own GPU program,
          // bypass the event loop of OF and call myOwnDraw() directly
		  // make ofApp::draw() which is the builtin draw callback of OF empty

       // bool isFrameBufferRedrawn;
		//appPtr-> myOwnDraw( &isFrameBufferRedrawn); 
         //appPtr-> myOwnDraw(); 
		 //glFinish(); 

		// added by Moon Jung, 2014/8/9.
		// The drawing of the scene is treated separately from other
		// standard draw events; ofApp::draw() is empty
		
	    //  When all the draw events are handled, call glfwSwapBuffers(windowP),
		// which is executed in display()
		//}
...
void ofAppGLFWWindow::display(void){
	...
	// in our case where I used my own GPU programming, renderer is an empty object
	if(renderer){
		renderer->startRender(); 
		// if the renderer is a GLProgrammable renderer, this step is required
		// => glUseProgram( program ) and load uniform matrices; this program is 
		// different from that I use in when I draw the scene with rainbow
	}

	// set viewport, clear the screen
	ofViewport();		// used to be glViewport( 0, 0, width, height );
	                     // ofViewport() has default parameters in all parameters
	                     // it uses the default renderer (static) which is defined in Graphics.cpp
	...
	// background is not cleared every display cycle, it is
	 // cleared in myOwnDraw(), which draws the scene.
	...
	// it calls renderer->ofSetupScreen(); where renderer object is the default one or the GLProgrammable one
