...
//--------------------------------------------------------------
void ofShader::begin() { // to draw the scene, use shader.begin() .... shader.end(); from nextShader.begin() 
	if (bLoaded){
		glUseProgram(program);
		ofPtr<ofGLProgrammableRenderer> renderer = ofGetGLProgrammableRenderer();
		if(renderer){
			renderer->beginCustomShader(*this); // *this shader is may be a real custom shader or
			               // a defaultShader, that is, "custom shader" created by the system
			                // distinguished by settingDefaultShader flag 
		}
	}else{
