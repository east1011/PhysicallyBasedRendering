...
//----------------------------------------------------------
void ofGLProgrammableRenderer::beginCustomShader(ofShader & shader){ //<= shader.begin()
...
void ofGLProgrammableRenderer::beginDefaultShader(){ 
	// <= ::startRender(), <= enableTextureTarget()
	...
	if(nextShader && (!currentShader || *currentShader!=*nextShader)){
		settingDefaultShader = true; // nextShader is a Default Shader!
		nextShader->begin(); // nextShader object distinguish defaultShader from customShader
		settingDefaultShader = false;
	}else{
	}
}
